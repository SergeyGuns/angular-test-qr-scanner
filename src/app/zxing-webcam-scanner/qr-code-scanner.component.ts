...
export class QrCodeScannerComponent implements AfterViewInit, OnDestroy {
  ...
  private decodeCallBack(result: Result): void {
    ...
    const charsetAttempts: Charset[] = ["UTF8", "Cp1251", "KOI8_R", "Cp1252", null];
    const allCharsetTextResults: [Charset, Result][] = charsetAttempts.map(
      (charset) => {
  ...
  private fixResultScanner(
    result: Result,
    charset?: Charset,
    status?: CHEKING_STATUS
  ): void {
    let requestResultBody: IQrCodeParam[] = [
      { key: "RawQr", value: result.getText() },
    ];
    ...
    if (
      status === CHEKING_STATUS.GOST_OK ||
      status === CHEKING_STATUS.NSPK_OK
    ) {
      this.resetEncode();
      this.onClose();
      this.operationRunService.startOperationByScenarioResultModal(
        this.wikiPayApiService.wikiPayLaunchRoublePaymentOperationByQr2(
          requestResultBody
        )
      );
    }
    ...
    !environment.production &&
      console.log({ requestResultBody, result, charset, status });
  }
